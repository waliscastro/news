									...
									foreach ( $required_plugins as $key => $value ):
										...
										if ( file_exists( WP_PLUGIN_DIR . '/' . $value['slug'] ) && is_plugin_inactive( $value['slug'] ) ) {
											...
											if( isset( $value['install'] ) &&  $value['install'] === 'locally' ) {
												...
												$button_note  = sprintf( esc_html__( 'Some plugins required manual installation. %1s zip comes with matina-pro bundle. Please install plugin zip in your site and reload this page for further actions.', 'mysterythemes-demo-importer' ), $value['name'] );
